...
function EditPopup({ edittoggle, setEditToggle }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const item_res = await interceptor.get(
          `${process.env.NEXT_PUBLIC_BASE}/getstock/?id=${edittoggle.id}`
        );
  ...
  async function handleSubmit() {
    try {
      ...
      const res = await interceptor.post(
        `${process.env.NEXT_PUBLIC_BASE}/updatestock`,
        formData,
        {
          headers: {
      ...
      <div className=" w-full h-full rounded-2xl border-[1px] border-[#E9E9E9] relative py-12 px-4 ">
        ...
        <div className="absolute top-2 right-4">
          <Image
            ...
            alt="closebtn"
            width={24}
            height={24} className="hover:scale-110 cursor-pointer"
             onClick={() => {
              console.log("clicked edit");
              setEditToggle({ tog: false, id: 0 });
              }}
